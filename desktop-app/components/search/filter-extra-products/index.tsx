...
function ExtraProductFilterNavBar({isOpen, toOpen}:timeFilterProps) {
  ...
  return (
    <div className={styles.option}>
        <p onClick={()=>toOpen('extraProduct')} className={styles.optionTittle}><FormattedMessage defaultMessage= "Servicios Extras"/></p>
        ...
        >
            <CardsReelSection
                ...
                gap={10}
            >
                {(e) =><p onClick={()=>setCountriesChecked(e.value)}>{e.label}</p>}
            </CardsReelSection>    
          </PaymentMethodFormElement>
        </div>
    </div>
