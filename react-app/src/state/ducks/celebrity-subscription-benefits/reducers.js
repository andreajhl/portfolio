...
const thunkPublicListSubscriptionBenefitsReducer = thunkReducer(
  types.PUBLIC_LIST_SUBSCRIPTION_BENEFITS_REQUEST,
  {
    totalResults: 0,
    results: [],
    currentParams: { offset: 0 },
...
const publicListSubscriptionBenefitsReducer = (state, action) => {
  if (action.type === types.SET_PUBLIC_LIST_SUBSCRIPTION_BENEFITS_OFFSET) {
    return {
      ...state,
      data: {
        ...state.data,
  ...
  return thunkPublicListSubscriptionBenefitsReducer(state, action);
};

export default combineReducers({
  listSubscriptionBenefitsReducer,
  publicListSubscriptionBenefitsReducer,
});
