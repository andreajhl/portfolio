...
const deliveryTimeFilter = [
  {
    label: defineMessage({ defaultMessage: "Entrega Flash 24 hrs" }),
    value: "flash",
  },
...
const getCheckItemLabel = (activeValue) =>
  deliveryTimeFilter.find(({ value }) => value === activeValue)?.label || "";

export const FilterSeccionDeliveryTime = ({
  setFilterByDeliveryTimeParam,
  queryParams,
  ...
  return (
    <ModalSelect
      buttonLabel={intl.formatMessage(
        messageForLabelButtonCategoryDeliveryTime
      )}
      ...
      options={deliveryTimeFilter.map((e) => ({
        label: intl.formatMessage(e.label),
        value: e.value,
      }))}
      showSearch={false}
