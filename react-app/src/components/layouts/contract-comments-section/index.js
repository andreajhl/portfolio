...
class ContractCommentSectionLayout extends Component {
    ...
    this.scrollDiv.current.addEventListener("scroll", () => {
      if (
        this.scrollDiv.current.scrollTop +
          this.scrollDiv.current.clientHeight >=
  ...
  listContractComments() {
    this.props.listContractComments(
  ...
  renderContractComments() {
    if (this.props.contractComments.length) {
      return this.props.contractComments.map((comment, index) => {
        return (
          <div
            className="item mr-4 mb-2 mx-auto"
            key={index + "-" + comment.id}
          >
            <ContractCommentCardLayout contractComment={comment} />
          </div>
        );
      });
    } else {
...
// Export Class
const _ContractCommentSectionLayout = connect(
  mapStateToProps,
  mapDispatchToProps
)(ContractCommentSectionLayout);
