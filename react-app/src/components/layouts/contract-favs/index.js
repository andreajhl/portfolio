...
import {
  addOrRemoveContractLike,
  getContractLikesData
...
class ContractFavsLayout extends Component {
  ...
  componentDidMount() {
    getContractLikesData(this.props.contractReference).then((data) => {
      this.setState({
        ...this.state,
  ...
  addOrRemoveFav() {
    if (this.session.getSession()) {
      addOrRemoveContractLike(this.props.contractReference).then((data) => {
        this.setState({
          ...this.state,
          markedByMe: data.markedByMe,
          favCount: data.count
    ...
    return (
      <div className="ContractFavsLayout">
        <i
          className={
            "fa fa-2x fa-heart" +
            (this.state.markedByMe ? " text-primary " : "")
          }
