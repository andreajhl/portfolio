export const AVAILABLE_CURRENCIES = [
  {
    name: "ARS",
    label: "Pesos Argentinos",
    symbol: "$",
    implemented_by_dlocal: true,
    flag: "https://restcountries.com/data/arg.svg",
    document_name: "DNI o CUIT",
    decimal_separator: ",",
    thousand_separator: ".",
    round: "100",
  },
  {
    name: "BRL",
    label: "Real Brasileño",
    symbol: "$",
    implemented_by_dlocal: true,
    flag: "https://restcountries.com/data/bra.svg",
    document_name: "CPF o CNPJ",
    decimal_separator: ",",
    thousand_separator: ".",
    round: "5",
  },
  {
    name: "CAD",
    label: "Dólares Canadienses",
    symbol: "$",
    implemented_by_dlocal: false,
    flag: "https://datosmacro.expansion.com/img/flag/CAg.png",
    document_name: "SIN",
    decimal_separator: ".",
    thousand_separator: ",",
    round: "1",
  },
  {
    name: "CLP",
    label: "Pesos Chilenos",
    symbol: "$",
    implemented_by_dlocal: false,
    flag: "https://restcountries.com/data/chl.svg",
    document_name: "CI/RUT",
    decimal_separator: ",",
    thousand_separator: ".",
    round: "1000",
  },
  {
    name: "COP",
    label: "Pesos Colombianos",
    symbol: "$",
    implemented_by_dlocal: true,
    flag: "https://restcountries.com/data/col.svg",
    document_name: "CC",
    decimal_separator: ",",
    thousand_separator: ".",
    round: "1000",
  },
  {
    name: "EUR",
    label: "Euros",
    symbol: "€",
    implemented_by_dlocal: false,
    flag:
      "https://upload.wikimedia.org/wikipedia/commons/thumb/b/b7/Flag_of_Europe.svg/1200px-Flag_of_Europe.svg.png",
    document_name: "DNI",
    decimal_separator: ",",
    thousand_separator: ".",
    round: "1",
  },
  {
    name: "DOP",
    label: "Pesos Dominicanos",
    symbol: "$",
    implemented_by_dlocal: false,
    flag: "https://restcountries.com/data/dom.svg",
    document_name: "Cédula",
    decimal_separator: ",",
    thousand_separator: ".",
    round: "50",
  },
  {
    name: "MXN",
    label: "Pesos Mexicanos",
    symbol: "$",
    implemented_by_dlocal: true,
    flag: "https://restcountries.com/data/mex.svg",
    document_name: "CURP",
    decimal_separator: ".",
    thousand_separator: ",",
    round: "5",
  },
  {
    name: "PEN",
    label: "Sol",
    symbol: "S/",
    implemented_by_dlocal: false,
    flag: "https://restcountries.com/data/per.svg",
    document_name: "DNI",
    decimal_separator: ",",
    thousand_separator: ".",
    round: "5",
  },
  {
    name: "PYG",
    label: "Pesos Paraguayos",
    symbol: "$",
    implemented_by_dlocal: false,
    flag: "https://restcountries.com/data/pry.svg",
    document_name: "CI",
    decimal_separator: ",",
    thousand_separator: ".",
    round: "10000",
  },
  {
    name: "USD",
    label: "Dólares",
    symbol: "$",
    implemented_by_dlocal: false,
    flag: "/assets/img/usa.svg",
    document_name: "SSN",
    decimal_separator: ".",
    thousand_separator: ",",
    round: "1",
  },
  {
    name: "UYU",
    label: "Pesos Uruguayos",
    symbol: "$",
    implemented_by_dlocal: false,
    flag: "https://restcountries.com/data/ury.svg",
    document_name: "CI",
    decimal_separator: ",",
    thousand_separator: ".",
    round: "50",
  },
  {
    name: "BOB",
    label: "Boliviano",
    symbol: "Bs.",
    implemented_by_dlocal: true,
    flag: "https://restcountries.com/data/bol.svg",
    document_name: "CI",
    decimal_separator: ",",
    thousand_separator: ".",
    round: "7",
  },
  {
    name: "CRC",
    label: "Colón costarricense",
    symbol: "₡",
    implemented_by_dlocal: true,
    flag: "https://restcountries.com/data/cri.svg",
    document_name: "CI",
    decimal_separator: ",",
    thousand_separator: ".",
    round: "626",
  },
  {
    name: "GTQ",
    label: "Quetzal",
    symbol: "Q",
    implemented_by_dlocal: true,
    flag: "https://restcountries.com/data/gtm.svg",
    document_name: "CUI",
    decimal_separator: ",",
    thousand_separator: ".",
    round: "7",
  },
];
