...
import { removeSentryUser, setSentryUser } from "./sentry";
...
export async function validateEmailSecurityCode({ email, securityCode }) {
  const response = await axios.post("/api/validate-security-code", {
    email: email?.trim?.().toLowerCase?.(),
    securityCode: securityCode?.trim?.()?.toLowerCase?.(),
  });
...
export const famososAuthService = axios.create({
  baseURL: `${endpoint}/${version}/famosos-com`,
});
