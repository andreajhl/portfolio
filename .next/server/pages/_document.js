...
// EXTERNAL MODULE: external "isbot"
var external_isbot_ = __webpack_require__("zlZz");
var external_isbot_default = /*#__PURE__*/__webpack_require__.n(external_isbot_);

// CONCATENATED MODULE: ./react-app/src/utils/debug.ts
/**
...
/* harmony default export */ var utils_debug = (debug);
// CONCATENATED MODULE: ./lib/setUserLocationCookie.ts
...
function getUserIp(request) {
  let userIp = request.headers["x-forwarded-for"] || request.connection.remoteAddress;
...
async function getIpCountryCode(userIp) {
  const response = await external_axios_default.a.get(`http://api.ipstack.com/${userIp}?access_key=ac1c0a88db0de9da13fcdba5d6742384&fields=country_code`);
...
const getUserLocationCountryCode = async (request) => {
  try {
    ...
    utils_debug("Se va a llamar a IPStack con la IP", userIp);
...
async function setLocationCookieHeader(req, res) {
  const userLocationValue = await getUserLocationCountryCode(req);
  res.setHeader("Set-Cookie", Object(external_cookie_["serialize"])(keys["a" /* USER_LOCATION_KEY */], userLocationValue, {
...
const setUserLocationCookie = async ({
  ...
  if (external_isbot_default()(req.headers["user-agent"])) {
    return utils_debug("Este es un bot solicitando", req.url);
  }

  const cookies = Object(external_cookie_["parse"])((req === null || req === void 0 ? void 0 : (_req$headers = req.headers) === null || _req$headers === void 0 ? void 0 : _req$headers.cookie) || "");
  if (keys["a" /* USER_LOCATION_KEY */] in cookies) return;
  await setLocationCookieHeader(req, res);
};

/* harmony default export */ var lib_setUserLocationCookie = (setUserLocationCookie);
...
class _document_MyDocument extends document_default.a {
  static async getInitialProps(ctx) {
    ...
    await lib_setUserLocationCookie(ctx);
    return initialProps;
  }

  render() {
    const isProdEnvironment = "qa" === "production";
    return __jsx(next_document["Html"], null, __jsx(next_document["Head"], null, __jsx("meta", {
      ...
      src: "https://www.facebook.com/tr?id=430558828300567&ev=PageView &noscript=1"
    })), __jsx("script", {
      dangerouslySetInnerHTML: {
        __html: `
      ...
      className: "f-main-body"
    }, __jsx(next_document["Main"], null), __jsx(next_document["NextScript"], null)));
  }
